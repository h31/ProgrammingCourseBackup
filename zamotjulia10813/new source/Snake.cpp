...
Zmeika::Zmeika(int X, int Y, int Napx, int Napy,int LTail)
{
    ...
    for(int i=0;i<LTail;i++)
    {
		...
		Tail2[i]->set_napx(this->napx); //направление элементов совпадает с направлением головы
...
//изменение направления элементов хвоста 
void Zmeika::shift() {


    for (unsigned int i=Tail2.size()-1; i>=1; i--) { //для остальных элементов присваиваем направление предыдущих
        Tail2[i]->set_napx(Tail2[i-1]->getnapx());
        Tail2[i]->set_napy(Tail2[i-1]->getnapy());
    }
	Tail2[0]->set_napx(this->napx); //присвоим направление для 1 элемента хвоста как у головы
...
ZmElement* Zmeika::getElementTail2(int Number)
{
	if(Number>0 && Number<Tail2.size())
		return Tail2[Number];	//возвращаем нужный элемент
	else return Tail2[Tail2.size()-1]; //иначе  - последний
}
