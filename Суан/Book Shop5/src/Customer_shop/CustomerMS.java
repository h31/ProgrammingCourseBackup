...
public class CustomerMS extends JFrame implements ActionListener, TreeSelectionListener, ListSelectionListener{
    ...
    /**
    * This function is executed when the program starts running
    */
    void LoadDB(){
        try {
    ...
    */
    ...
    void TaoHeader(){
        JLabel lbl=new JLabel("CUSTOMER OF SHOP MANAGMENT SYSTEM",JLabel.CENTER);
    ...
    //Biuld Tree
    root=new DefaultMutableTreeNode("List Customer");        
    ...
    JSplitPane spPane=new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,pLeft,pCen);       //dividing line
    ...
    b6.add(btSave=new JButton("Save"));btSave.addActionListener(this);btSave.setEnabled(false);     
    ...
    if(o.equals(btAddTypeCus)) {
        String sID=JOptionPane.showInputDialog("Input ID of TypeCus");                    
...
//            }
        DefaultMutableTreeNode selNode=(DefaultMutableTreeNode)tree.getSelectionPath().getLastPathComponent();
           ...
           treeModel.removeNodeFromParent(selNode);
            ...
            {
            if(db.getTypeCusList().get(i).getTypeID().equalsIgnoreCase(tem[0]))
            {
        ...
        if(btAdd.getText().equalsIgnoreCase("Add")){
            ...
            btAdd.setText("Add");                         //move  Delete->Add
...
//------------------------------------- 
        ...
        if(btAdd.isEnabled()){                                                         
            ...
            for(int i=0;i<db.getTypeCusList().size();i++)
            {
            if(db.getTypeCusList().get(i).getTypeID().equalsIgnoreCase(IDShelf))
            ...
            if(selTypeCus!=null) {
                Customer c=(Customer)lstCus.getSelectedValue();
                Customer b=new Customer(tfID.getText(),tfName.getText(),
                tfAddress.getText(),tfPhone.getText());
                if(!selTypeCus.UpdateCusInfos(c, b))
        ...
        Customer b=(Customer)lstCus.getSelectedValue();
    ...
    public void valueChanged(ListSelectionEvent arg0) {
        ...
        try {
            ...
            System.out.println(selBook.toString());
            if(selBook!=null) {
