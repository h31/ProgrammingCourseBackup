...
public:
	...
	HGETileMap(int levelwidth, int levelheight)
	{
		drawX = 0;
		drawY = 0;
	...
	void LoadServerMap(std::string);//без текстур, только коллиззии
	...
	int GetTileOffset_x(){return (vpX / TILE_W);}//new
	int GetTileOffset_y(){return (vpY / TILE_H);}//new
	hgeVector GetOffset(){return hgeVector(vpX,vpY);}//new


	// Returns the map size in pixels
	void GetMapSize(int *, int *);

	int GetOffset_x(){return vpX;}
	int GetOffset_y(){return vpY;}
	...
	float Sign(hgeVector p1, hgeVector p2, hgeVector p3)
	{
		return (p1.x - p3.x) * (p2.y - p3.y) - (p2.x - p3.x) * (p1.y - p3.y);
	}

	bool IsPointInTri(hgeVector pt, hgeVector v1, hgeVector v2, hgeVector v3)
	{
	  bool b1, b2, b3;
	...
	bool Frame();
	void Render();
	void Render_with_fof(hgeVector,hgeVector,hgeVector);//pos,r left, r right
	void RenderFG();
