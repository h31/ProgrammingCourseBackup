...
void HGETileMap::Render_with_fof(hgeVector pos,hgeVector r1,hgeVector r2)
{
		...
		for (int cy=ytile; cy<ytile+vpHeight; cy++)
			for (int cx=xtile; cx<xtile+vpWidth; cx++)
				if (tiles[cx+(cy*info.mapwidth)])
				{
					...
					if (IsPointInTri(hgeVector(	drawX-px + (cx-xtile)*TILE_W, drawY - py + (cy-ytile)*TILE_H ),pos,r1+pos,r2+pos))
					{
					if (tiles[cx+(cy*info.mapwidth)]->isBackground())
						tiles[cx+(cy*info.mapwidth)]->background->Render(	drawX-px + (cx-xtile)*TILE_W, drawY - py - (TILETEX_H-TILE_H) + (cy-ytile)*TILE_H );
		...
		for (int cy=ytile; cy<ytile+vpHeight; cy++)
			for (int cx=xtile; cx<xtile+vpWidth; cx++)
				if (tiles[cx+(cy*info.mapwidth)])
					...
					{
						//tiles[cx+(cy*info.mapwidth)]->foreground->SetBlendMode(BLEND_COLORMUL | BLEND_ALPHABLEND | BLEND_NOZWRITE);
...
void HGETileMap::LoadServerMap(std::string filename)
{
	long i, j;
...
void HGETileMap::LoadMap(std::string filename)
{
	...
	for (i=0; i<info.nTiles-1; i++)
	{
		...
		if (tiles[j]->fgTexID > 0)
		{
			...
			//tiles[j]->foreground->SetBlendMode(BLEND_COLORADD);
			tiles[j]->foreground->SetColor(0xDF000000);
		}
