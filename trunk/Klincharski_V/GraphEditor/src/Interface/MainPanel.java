...
class MainPanel extends JPanel implements InfoListener {
    ...
    private void onPressAddCity(int x, int y) {
        vertice city = new vertice("Узел", x, y);
    ...
    private void onPressAddPointFinish(int x, int y)
    {
        ...
        if (finishCity != null && finishCity != startCity &&
            ...
            if (newWayKind != null) {
                ...
                JOptionPane.showMessageDialog(this, "Минимальный путь из вершины " + startCity.getName() + " в " + finishCity.getName() + " равен " + String.valueOf(minpath), "Минимальный путь из вершины " + startCity.getName() + " в " + finishCity.getName(), JOptionPane.INFORMATION_MESSAGE);
                currentCity = finishCity;
                currentListener.currentCityChanged(finishCity);
                //currentWay = newWay;
                //currentListener.currentWayChanged(newWay);
