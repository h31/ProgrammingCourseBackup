...
public class BookMS extends JFrame implements ActionListener, TreeSelectionListener, ListSelectionListener{
    ...
    public BookMS() {
        ...
        TaoGUI();
        LoadDB();
        LockText(true);//can not edit the information
        }
    /**
    ...
    */
    ...
    void TaoHeader(){
        JLabel lbl=new JLabel("BOOK OF SHOP MANAGMENT SYSTEM",JLabel.CENTER);
       ...
       // }
          DefaultMutableTreeNode selNode=(DefaultMutableTreeNode)tree.getSelectionPath().getLastPathComponent();
           ...
           treeModel.removeNodeFromParent(selNode);
            ...
            {
            if(db.getShelfList().get(i).getShelfID().equalsIgnoreCase(tem[0]))
            {
        ...
        if (pages<=0){JOptionPane.showMessageDialog(null,"Input positive number of pages");
        return;}
//------------------------------------- 
        double price=0.0;
        try {
        ...
        if (price<=0){JOptionPane.showMessageDialog(null,"Input positive number of price's book");
        return;}
        if(btAdd.isEnabled()){                                     //nút lưu của nút thêm
          ...
          for(int i=0;i<db.getShelfList().size();i++)
            {
            if(db.getShelfList().get(i).getShelfID().equalsIgnoreCase(IDShelf))
            {
                db.getShelfList().get(i).AddBook(b);
            }
           // System.out.println(IDShelf);
           // System.out.println(db.getShelfList().get(i).getShelfID());
            
            }
            dlmBook.addElement(b);
            for(int i=0;i<dlmBook.getSize();i++)System.out.println(dlmBook.getElementAt(i));
        ...
        Book b=(Book)lstBook.getSelectedValue();
        if(id.trim().equals("")) {
            JOptionPane.showMessageDialog(null, "Choose book");
            return;
            }
        if(JOptionPane.showConfirmDialog(null,
        "You sure you want to remove this book?",
        "OK",JOptionPane.YES_NO_OPTION,
        JOptionPane.QUESTION_MESSAGE)==JOptionPane.YES_OPTION) {

            if(!selShelf.RemoveBook(b))
