...
public class TestHand {
    ...
    TestHand(Card[] cards)
    {
        final int numOfCards = 7;
        final int ranksOfCards = 13;
        value = new int[6];
        ...
        for(int h = 0; h<numOfCards; h++){
            int flush_index = 1;
            for(int x = 0; x<numOfCards; x++) {
            if(h!=x){    
            if ( cards[h].getSuit() == cards[x].getSuit() )
            {
                flush_index++;
                if(flush_index >= 5)
                {
        ...
        if (sameCards==2 && sameCards2==2) //two pair
        {
            ...
            value[3]=orderedRanks[0];  //kicker
        }

        if (sameCards==3 && sameCards2!=2)    //tripse
        {
            value[0]=4;
        ...
        if (straight && flush)   //straigh flush
        {
            value[0]=9;
        }
